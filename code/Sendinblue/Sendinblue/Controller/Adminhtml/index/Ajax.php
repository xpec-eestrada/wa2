...
class Ajax extends \Magento\Backend\App\Action
{
    ...
    public function subsUnsubsContact()
    {
        ...
        if (!empty($email) && $postNewsLetter == 0) {
            $storeId = $model->storeMangInterface->getStore()->getId();
            $model->customerVal->setWebsiteId($storeId);
            $dataCust = $model->customerVal->loadByEmail($email);
            ...
            if (isset($customer['entity_id']) > 0) {
                ...
                $stores = $model->storeMangInterface->getStores(true, false);
