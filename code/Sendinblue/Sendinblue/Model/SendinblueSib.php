...
class SendinblueSib extends \Magento\Framework\Model\AbstractModel
{
    /** @var  ConfigHelper */
    public $resConfig;
    public $getValueDefault;
    public $scopeTypeDefault;
    public $dirVal;
    public $customerVal;
    public $subscribers;
    public $resources;
    public $storeMangInterface;
    ...
    public function __construct(
        ...
        $this->orderCollectionFactory = $salesOrderCollectionFactory;
        $this->resConfig = $resourceConfig;
        $this->getValueDefault = $scopeDefaultType;
        $this->scopeTypeDefault = $scopeDefaultType::SCOPE_TYPE_DEFAULT;
        $this->storeId = $defaultStoreId::DEFAULT_STORE_ID;
        $this->dirVal = $dir;
        $this->customerVal = $customers;
        $this->subscribers = $subscriber;
        $this->resources = $resource;
        $this->storeMangInterface = $storeManagerInterface;
        $this->getTbVal = $getTb;
    }
    ...
    public function createNewList($response, $existList)
    {
        ...
        $this->resConfig->saveConfig('sendinblue/selected_list_data', trim($listResp['data']['id']), $this->scopeTypeDefault, $this->storeId);
        //list id
    ...
    {
        if (!$key) {
            $key = $this->getValueDefault->getValue('sendinblue/api_key', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        }
    ...
    {
        $this->resConfig->saveConfig('sendinblue/ord_track_status', 0, $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/order_import_status', 0, $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/api_smtp_status', 0, $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/first_request', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/api_key', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/selected_list_data', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/confirm_type', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/doubleoptin_redirect', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/optin_url_check', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/final_confirm_email', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/optin_list_id', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/final_template_id', '', $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/api_sms_shipment_status', 0, $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/api_sms_campaign_status', 0, $this->scopeTypeDefault, $this->storeId);
        $this->resConfig->saveConfig('sendinblue/api_sms_order_status', 0, $this->scopeTypeDefault, $this->storeId);
    }
    ...
    public function createAttributesName()
    {
        ...
        if ($valueLanguage->language == 'fr') {
            $dataAttr = ['PRENOM'=>'text','NOM'=>'text','CLIENT'=>'number','COMPANY'=>'text','SMS'=>'text','CITY'=>'text','COUNTRY'=>'text','POSTCODE'=>'text','PROVINCE_CODE'=>'text','COUNTRY_CODE'=>'text'];
        } else {
            $dataAttr = ['NAME'=>'text','SURNAME'=>'text','CLIENT'=>'number','COMPANY'=>'text','SMS'=>'text','CITY'=>'text','COUNTRY'=>'text','POSTCODE'=>'text','PROVINCE_CODE'=>'text','COUNTRY_CODE'=>'text'];
        }
        ...
        $transactionalAttributes = ['ORDER_ID'=>'id','ORDER_DATE'=>'date','ORDER_PRICE'=>'number'];
        $trans = ["type" => "transactional",
        "data" => $transactionalAttributes
        ];
        $mailin->createAttribute($trans);

        $calculatedValue = ["type" => "calculated",
        "data" => $calcAttribute];
        $mailin->createAttribute($calculatedValue);

        $dataGlobal = ["type" => "global",
    ...
    {
        $userLanguage = $this->getValueDefault->getValue('sendinblue/sendin_config_lang', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        if ($userLanguage == 'fr') {
            $attributesName = ['PRENOM'=>'firstname', 'NOM'=>'lastname', 'MAGENTO_LANG'=>'created_in','CLIENT'=>'client','SMS'=>'telephone','COMPANY'=>'company','CITY'=>'city','COUNTRY_ID'=>'country_id','POSTCODE'=>'postcode','STREET'=>'street','REGION'=>'region','STORE_ID'=>'store_id'];
        }
        else {
            $attributesName = ['NAME'=>'firstname', 'SURNAME'=>'lastname', 'MAGENTO_LANG'=>'created_in','CLIENT'=>'client','SMS'=>'telephone','COMPANY'=>'company','CITY'=>'city','COUNTRY_ID'=>'country_id','POSTCODE'=>'postcode','STREET'=>'street','REGION'=>'region','STORE_ID'=>'store_id'];
        }
    ...
    {
        if ($langConfig == 'fr') {
            $attributesType = ['PRENOM'=>'text', 'NOM'=>'text', 'MAGENTO_LANG'=>'text','CLIENT'=>'number','SMS'=>'text','COMPANY'=>'text','CITY'=>'text','COUNTRY_ID'=>'text','POSTCODE'=>'number','STREET'=>'text','REGION'=>'text','STORE_ID'=>'number'];
        } else {
            $attributesType = ['NAME'=>'text', 'SURNAME'=>'text', 'MAGENTO_LANG'=>'text','CLIENT'=>'number','SMS'=>'text','COMPANY'=>'text','CITY'=>'text','COUNTRY_ID'=>'text','POSTCODE'=>'number','STREET'=>'text','REGION'=>'text','STORE_ID'=>'number'];
        }
        return $attributesType;
    }

    public function attrCalculated() {
        $calcAttr = ['[{ "name":"MAGENTO_LAST_30_DAYS_CA", "value":"SUM[ORDER_PRICE,ORDER_DATE,>,NOW(-30)]" }, {"name":"MAGENTO_ORDER_TOTAL", "value":"COUNT[ORDER_ID]"}, {"name":"MAGENTO_CA_USER", "value":"SUM[ORDER_PRICE]"}]'];
        return $calcAttr;
    }

    public function attrGlobal() {
        $globalAttr = ['[{ "name":"MAGENTO_CA_LAST_30DAYS", "value":"SUM[MAGENTO_LAST_30_DAYS_CA]" }, { "name":"MAGENTO_CA_TOTAL", "value":"SUM[ORDER_USER]"}, { "name":"MAGENTO_ORDERS_COUNT", "value":"SUM[MAGENTO_ORDER_TOTAL]"}]'];
        return $globalAttr;
    }
    /**
    ...
    public function sendAllMailIDToSendin($listId)
    {
        ...
        $baseUrl = $this->storeMangInterface->getStore()->getBaseUrl();
        $apiKey = !empty($this->apiKey) ? $this->apiKey : $this->getDbData('api_key');
    ...
    public function getSubscribeCustomer()
    {
        ...
        foreach ($resultSubscriber as $subsdata){
            ...
            if ( !empty($customerAddressData[$subscriberEmail]) ) {
                ...
                $stores = $this->storeMangInterface->getStores(true, false);
        ...
        if (!is_dir($this->dirVal->getPath('media').'/sendinblue_csv')) {
            mkdir($this->dirVal->getPath('media').'/sendinblue_csv', 0777, true);
        }

        $handle = fopen($this->dirVal->getPath('media').'/sendinblue_csv/ImportSubUsersToSendinblue.csv', 'w+');
    ...
    public function getCollection()
    {
        ...
        return $this->customerVal->getCollection();
    }

    public function getCustomer($customerId)
    {
        //Get customer by customerID
        $dataCust = $this->customerVal->load($customerId);
    ...
    {
        return $this->getValueDefault->getValue('sendinblue/'.$val, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
    }
    ...
    {
        $this->resConfig->saveConfig('sendinblue/'.$key, $value, $this->scopeTypeDefault, $this->storeId);
    }
    ...
    {
        if (!$apiKey) {
            $apiKey = $this->getValueDefault->getValue('sendinblue/api_key', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        }
    ...
    public function updateSender()
    {
        ...
        if (!empty($apiKey)) {
            ...
            if ($response['code'] == 'success') {
                $senders = ['id' => $response['data']['0']['id'], 'from_name' => $response['data']['0']['from_name'], 'from_email' => $response['data']['0']['from_email']];
    ...
    public function importOrderhistory()
    {
        ...
        if ($trackStatus == 1 && !empty($apiKey)) {
            ...
            $handle = fopen($this->dirVal->getPath('media').'/sendinblue_csv/ImportOldOrdersToSendinblue.csv', 'w+');
                ...
                if (isset($resultSubscriber[0]['countval']) && $resultSubscriber[0]['countval'] > 0) {
                    ...
                    foreach ($order as $orderDatamodel) {
                        if (count($orderDatamodel) > 0) {
                            ...
                            $historyData= [];
                            $historyData[] = [$email, $orderID, $orderPrice, $orderDate];
    ...
    public function sendOrderTestSms($sender, $message, $number)
    {
        ...
        if(!empty($number) && !empty($apiKey)) {
            ...
            $currency = $this->storeMangInterface->getStore()->getCurrentCurrencyCode();
    ...
    public function multipleChoiceCampaign($post)
    {
        ...
        if (!empty($senderCampaign) && !empty($senderCampaignMessage) && $smsCredit >= 1) {
            ...
            $customerObj = $this->customerVal->getCollection();
    ...
    {
        return $connection = $this->resources->getConnection(\Magento\Framework\App\ResourceConnection::DEFAULT_CONNECTION);
    }
    ...
    public function sendNotifySms($remainingSms)
    {
        ...
        $siteName = $this->storeMangInterface->getStore()->getName();
        $paramVal = ['{present_credit}' => $remainingSms, '{site_name}' => $siteName];
    ...
    public function sendWsTemplateMail($to)
    {
        ...
        if ($sendinConfirmType == 'doubleoptin') {
            ...
            $path = $this->storeMangInterface->getStore()->getBaseUrl().'sendinblue';
            $siteName = $this->storeMangInterface->getStore()->getName();
            $pathResp = $path.'?value='.base64_encode($to);

            $title = __('Please confirm your subscription');

            $paramVal = ['{double_optin}' => $pathResp];
            ...
            $headers = ["Content-Type"=> "text/html;charset=iso-8859-1", "X-Mailin-tag"=>$transactionalTags ];
            $data = ["to" => $to,
                "cc" => [],
                "bcc" =>[],
                "from" => $from,
    ...
    public function getNewsletterSubscribe($start, $perPage)
    {
        ...
        if (count($connection->fetchAll($query)) > 0) {
            foreach ($connection->fetchAll($query) as $emailValue) {
                ...
                $storeId = $this->storeMangInterface->getStore()->getId();
                $this->customerVal->setWebsiteId($storeId);
                $dataCust = $this->customerVal->loadByEmail($email);
    ...
    {
        return $this->getTbVal->getTable($tableName);
    }
//end class
}
