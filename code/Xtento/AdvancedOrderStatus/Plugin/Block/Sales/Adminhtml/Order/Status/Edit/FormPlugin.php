 ...
 * ID:            %!uniqueid!%
 * Packaged:      %!packaged!%
 * Last Modified: 2017-09-01T16:50:28+00:00
 * File:          Plugin/Block/Sales/Adminhtml/Order/Status/Edit/FormPlugin.php
...
class FormPlugin
{
    ...
    public function afterSetForm(Form $subject, $interceptedOutput)
    {
        ...
        $baseFieldset->addType('color_picker', '\Xtento\AdvancedOrderStatus\Block\Adminhtml\Element\ColorPicker');
        $baseFieldset->addField(
            'color',
            'color_picker',
            [
                'label' => __('Status Color'),
                'note' => __('If you want to remove the color for an order status, set it to #000000 (the darkest black you can find)')
            ]
        );
